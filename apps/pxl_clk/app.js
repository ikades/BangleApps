...

{
  ...
  let draw = function() {
    ...
    if(settings.d_or_i == "date"){
      ...
      g.setFontAlign(-1, -1).setFont("Ikades:2").drawString(d.getDate(), 119, 86);
      g.setFontAlign(-1, -1).setFont("Ikades").drawString(require("locale").month(new Date(), 2).toUpperCase(), 139, 98);
    }
  ...
  let clockInfoDraw = (itm, info, options) => {
    ...
    var t_x = options.x+options.w/2, t_y = options.y+33;
    if (g.setFont("Ikades:2").stringWidth(text)+20>options.w){
      g.setFont("Ikades");
      t_y += 6;
    }
    g.setFontAlign(0,-1).drawString(text, t_x, t_y);
  };
